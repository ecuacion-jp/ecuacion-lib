...
public abstract class ConditionalValidator {
  ...
  public static Object getFieldValue(String fieldName, Object instance, String fieldKindName) {
    Method validationTargetFieldGetterMethod;
    String methodName = "get" + StringUtils.capitalize(fieldName);
    
    try {
      validationTargetFieldGetterMethod = instance.getClass().getMethod(methodName);

    } catch (Exception ex) {
      throw new EclibRuntimeException(
          "'" + fieldKindName + "' getter method instance cannot be obtained "
              + "from the method name '" + methodName + "'",
          ex);
    }

    try {
      return validationTargetFieldGetterMethod.invoke(instance);

    } catch (Exception ex) {
      throw new EclibRuntimeException(
          "The return value of '" + methodName + "' instance cannot be obtained.", ex);
    }
  }
